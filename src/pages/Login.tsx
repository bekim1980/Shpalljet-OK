  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: "https://shpalljet.net/",
        },
      });
        ...
        <div className="glass-card rounded-xl p-6 shadow-gold space-y-6">
          <div className="text-center space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              {isSignUp ? t("login.joinMarket") : t("login.signInToAccount")}
            </p>
          </div>
          ...
          <div className="space-y-3">
            <Button
              variant="outline"
              className="w-full h-11 gap-3 font-medium border-border/60 hover:bg-secondary/60"
