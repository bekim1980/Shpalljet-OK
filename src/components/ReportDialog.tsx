...
interface ReportDialogProps {
  reportedType: "product" | "user" | "ride";
  reportedId: string;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {triggerVariant === "icon" ? (
          ...
          <Button variant="ghost" size="sm" className="h-10 flex-col gap-1 text-xs text-muted-foreground hover:text-destructive">
            <TriggerIcon className="h-4 w-4" />
            {triggerLabel ?? "Raporto"}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="font-display">
            Raporto {reportedType === "product" ? "Artikullin" : reportedType === "ride" ? "Udhëtimin" : "Përdoruesin"}
          </DialogTitle>
        </DialogHeader>
